var mygl_old=mygl_old||{};

//

mygl_old.createContext=(canvas,params,onError)=>{
...
(()=>{
    ...
    mygl_old.setDrawStates=(gl,clear,inputStates)=>{
        gl.myDrawStates=gl.myDrawStates||{};
        var toStates={};
...
mygl_old.uniformsApply=(gl,prog)=>{
    gl.myUniforms=gl.myUniforms||[{}];
...
mygl_old.createVertBuf=(gl,data)=>{
...
mygl_old.createVertBufs=(gl,datas)=>{
    return datas.map(data => mygl_old.createVertBuf(gl,data));
}

mygl_old.createIndBuf=(gl,data)=>{
    var buf=gl.createBuffer();
    gl.bindVertexArray(null);
...
mygl_old.createVao=(gl,locs,sizes,types,vertBufs,indBuf)=>{
...
mygl_old.createStridedVao=(gl,locs,sizes,types,stride,offsets,vertBuf,indBuf)=>{
    var vao=gl.createVertexArray();
    gl.bindVertexArray(vao);
...
(()=>{
    ...
    mygl_old.getProgram=(gl,vsName,fsName)=>{
